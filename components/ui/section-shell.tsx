...
export default function SectionShell({
  ...
  return (
    <section id={id} className={`px-6 py-24 ${className}`.trim()}>
      <div className="mx-auto max-w-6xl">
        {eyebrow || title || description ? (
          <div className={`mb-12 flex max-w-3xl flex-col gap-4 ${alignmentClass}`}>
            ...
            {title ? (
              <h2 className="text-4xl font-extrabold tracking-tight text-[#080A07] md:text-5xl">
                {title}
              </h2>
            ) : null}
            {description ? (
              <p className="text-base leading-8 text-black/58 md:text-lg">{description}</p>
            ) : null}
          </div>
        ) : null}
